...
public class ResourceServiceImpl implements ResourceService {
    ...
    public CommonResponse delete(Long resourceId, Long userId) {
        ...
        // 2. 判断当前用户是否为资源上传者
        boolean allowDeleteFlag = false;
        List<User> admins = this.userMapper.selectList(new QueryWrapper<User>().eq("role_id", 1l));
        ...
        //若当前用户既不是管理员也不是资源上传者，则不能删除
        if (allowDeleteFlag == false) {
        ...
        // 4. 删除文件
        if (allowDeleteFlag) {
            //如果是文件夹，则顺带查出它的子文件夹及文件一同删除
            if (resource.getType() == 0) {
                //删除该目录下的子文件夹及文件
                for (Resource childResource : childResourceList) {
                    this.delete(childResource.getId(), userId);
                }
            }
            // 5. 删除服务器上的文件
            boolean deleteFlag = this.resourceUtils.delete(resource.getLocation());
            // 6. 删除t_course_resource表的记录
            this.courseResourceMapper.delete(new QueryWrapper<CourseResource>().eq("resource_id", resourceId));
            // 7. 删除t_resource表的记录
            this.resourceMapper.deleteById(resourceId);
        }
